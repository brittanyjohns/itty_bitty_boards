...
- **Art drawn with a communicator LIKENESS is PICKABLE, never a DEFAULT — it is
  ...
  builder's sub-pages are attached to no one, so every fill path passes
  `communicator_id` through to `GenerateImagesJob`; a picture already drawn for
  the same owner and fingerprint is reused (`Images::LikenessArt`) rather than
  paid for twice, while an explicit regenerate always draws a new one.
...
| `.claude-notes/scene-engine.md` | Scene engine: the `SceneTemplate` library (base image + front layer + calibrated slots) and `SceneComposition` (per-slot art for a board printable or a printable product), the slot shape, layer order (base, warped slots, front layer), `calibration_version` + render digest, the four art sources, the per-owner source allowlist and their render-time ownership checks, the Stimulus calibrator, `rake scenes:import_vendored`, AI scene generation + magenta slot detection (the upload path that needs no OpenAI, front-layer extraction, the mask-only inpaint copy-back), and why product art is never sent to an image model |
| `.claude-notes/printable-products.md` | Non-board printables (`PrintableProduct`, device tags first): named `artworks` / `downloads` attachments with blob-metadata labels, the shared `CanvaTemplatesValidator` allowlist (also KitPage's), `product_artwork` scene mockups and why an artwork in use can't be removed, the admin screens and the shared composition controller/views, and what the Etsy follow-up inherits |
