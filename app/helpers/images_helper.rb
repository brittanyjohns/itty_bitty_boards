    ...
    def display_image_for(image, user)
      ...
      puts "display_image_for: #{image.label} - #{user_image}\n"
        ...
        if !user_image&.attached?
          str += "<div class='w-100 h-100 text-gray-400 text-2xl font-bold grid justify-items-center items-center shadow mx-auto my-auto'><span class='mx-auto my-28'>#{image.label.upcase}</span></div>".html_safe
        else
