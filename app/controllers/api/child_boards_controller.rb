class API::ChildBoardsController < API::ApplicationController
  ...
  def index
    ...
    #   @query = params[:query]
    #   @boards = boards_for_child.child_made_with_scenarios.where("name ILIKE ?", "%#{params[:query]}%").order(name: :desc)
    #   @predefined_boards = Board.predefined.where("name ILIKE ?", "%#{params[:query]}%").order(name: :desc)
    # else
    #   @boards = boards_for_child.child_made_with_scenarios.order(created_at: :desc)
    #   @predefined_boards = Board.predefined.order(created_at: :desc)
    # end
    @boards = boards_for_child.child_made_with_scenarios.order(created_at: :desc)

    render json: { boards: @boards.map { |b| b.api_view(current_child) },
                   predefined_boards: @predefined_boards }
  end

  def child_boards
    # @boards = boards_for_child.child_made_with_scenarios_and_menus.order(created_at: :desc)
    @boards = boards_for_child.child_made_with_scenarios.order(created_at: :desc)
        ...
        images: board.board_images.map do |board_image|
          {
            ...
            src: board_image.image.display_image_url(current_child),
    ...
    @board_with_images = @board.images.map do |image|
      {
        ...
        # src: image.display_image(current_child)&.url || "https://via.placeholder.com/300x300.png?text=#{image.label_param}",
        src: image.display_image_url(current_child),
    ...
    @next_images = @image.next_images.map do |ni|
      {
        ...
        # src: ni.display_image(current_child)&.url || "https://via.placeholder.com/300x300.png?text=#{ni.label_param}",
        src: ni.display_image_url(current_child),
  ...
  def show
    ...
    @board_with_images = board.api_view_with_images(current_child)
    child_permissions = {
      can_edit: (board.child == current_child || current_child.admin?),
      can_delete: (board.child == current_child || current_child.admin?),
    }
  ...
  def save_layout
    ...
    board.reload
    render json: board.api_view_with_images(current_child)
  end
  ...
  def create
    ...
    @board.child = current_child
    @board.parent_id = child_signed_in? ? current_child.id : params[:parent_id]
  ...
  def update
    ...
    respond_to do |format|
      if @board.update(board_params)
        format.json { render json: @board.api_view_with_images(current_child), status: :ok }
      else
        format.json { render json: @board.errors, status: :unprocessable_entity }
      end
    end
  ...
  def add_image
    @board = Board.with_artifacts.find(params[:id])
    @found_image = Image.find_by(label: image_params[:label], child_id: current_child.id, private: true)
  ...
  def associate_image
    ...
    if @board.predefined && !current_child.admin?
  ...
  def clone
    ...
    @new_board.child = current_child
    @new_board.parent_id = current_child.id
    ...
    @new_board.save!
    render json: @new_board.api_view_with_images(current_child)
  end
