...
class Profile < ApplicationRecord
  ...
  # The scan tag (Communicators::GenerateScanTag) — the QR code, one optional
  # line of text, and nothing else. Generated on profile save like the device
  # tag, so its thumbnail is always there.
  has_one_attached :scan_tag_png
  has_one_attached :scan_tag_pdf

  # The care plan documents (Communicators::GenerateCarePlan), one per
  # [variant, size] pair the generator offers. Deliberately NOT generated by
  # #generate_attachments!; they're built on demand from the endpoint so a
  # profile save doesn't pay for more Grover renders. See
  # .claude-notes/safety-profiles.md.
  has_one_attached :care_plan_pdf
  has_one_attached :care_emergency_plan_pdf
  has_one_attached :care_plan_half_pdf
  has_one_attached :care_emergency_plan_half_pdf
  has_one_attached :care_emergency_plan_wallet_pdf

  # A thumbnail of each of those documents, rendered from the same HTML in the
  # same generator call and sharing its freshness signature. One per PDF, so
  # the [variant, size] lookup stays a single table. These are what the Print &
  # share tab displays; the Device tag's PNG has always worked this way, and
  # the care plan is the only printable that had a download button with nothing
  # to look at.
  has_one_attached :care_plan_preview_png
  has_one_attached :care_emergency_plan_preview_png
  has_one_attached :care_plan_half_preview_png
  has_one_attached :care_emergency_plan_half_preview_png
  has_one_attached :care_emergency_plan_wallet_preview_png

  # The line printed under the scan tag's QR code when the owner hasn't written
  # one of their own. It lives here, not in the ERB, so there is exactly one
  # default: the device tag carries two that disagree (the service's and an
  # inline fallback in its template), which is a trap worth not repeating. The
  # frontend renders this as a PLACEHOLDER and never prefills the field, so
  # "left it alone" stays distinguishable from "typed exactly that".
  SCAN_TAG_DEFAULT_NOTE = "If I'm lost, please scan the QR code.".freeze
  ...
  # Runs SYNCHRONOUSLY on every safety-profile save — an avatar upload, a theme
  # tweak — so every generator added here is paid for by an operation the owner
  # thinks of as instant. It used to build the Safety ID card too, at two more
  # headless-Chrome renders per save; that card is no longer offered on Print &
  # share (the care plan's wallet size carries the same emergency fields plus
  # the care lines, in a format someone can actually carry), so it is no longer
  # built up front. The generator, the endpoint and the attachments all still
  # exist — nothing already generated is orphaned, and re-adding the call is
  # the whole of the revert.
  def generate_attachments!
    Communicators::GenerateDeviceTag.call(self)
    Communicators::GenerateScanTag.call(self)
  end
