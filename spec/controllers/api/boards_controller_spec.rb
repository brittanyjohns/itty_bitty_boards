...
RSpec.describe API::BoardsController, type: :controller do
  ...
  #   before do
  #     zip_path = obf_zip_file.path
  #     puts "OBF zip file path in before block: #{zip_path}"
  #     Zip::File.open(zip_path) do |zip_file|
  #       zip_file.each do |entry|
  #         puts "Zip entry: #{entry.name}"
  #         if entry.name == "manifest.json"
  #           puts "Found manifest.json in zip"
  ...
  #     puts "\n\nOBF zip file path: #{obf_zip_file_path}\n\n"
  #     puts "\n\nOBF zip file exists?: #{File.exist?(obf_zip_file_path)}\n\n"
  #     obf_zip_file = File.open(obf_zip_file_path)
  #     puts "Opened OBF zip file: #{obf_zip_file.path}"
  #     extracted_data = OBF::OBZ.to_external(obf_zip_file, {})
  #     puts "Extracted OBZ data keys: #{extracted_data.keys}"
  #     file_name = File.basename(obf_zip_file.path)
  #     puts "OBF file name: #{file_name}"
  #     json_data = {
  ...
  #     }
  #     puts "OBZ import JSON data: #{json_data.keys}"
  #     importer = ObzImporter.new(json_data, current_user)
  #     puts "Initialized ObzImporter"
  #     result = importer.import
  #     puts "OBZ import result: #{result}"
  ...
  describe "POST #import_obf" do
    let(:obf_file_path) { Rails.root.join("spec", "data", "simple_board.obf") }
    let(:obf_file) { File.open(obf_file_path) }

    it "imports from OBF file" do
      post :import_obf, params: { file: obf_file }, as: :json

      expect(response).to have_http_status(:ok)
      json_response = JSON.parse(response.body)
      expect(json_response["message"]).to eq("OBF import started")
    end
  end
end
