...
- **A freshly cloned board's `board_images.any?` reads a stale counter cache — reload before asking.** `BoardImage belongs_to :board, counter_cache: true`, so `any?`/`empty?`/`size` resolve through `count_records`, which reads the owner's `board_images_count` COLUMN and never queries. `Board#clone_with_images` zeroes that attribute before saving and then creates each tile with `board_id=` rather than through the association, so the DB counter climbs while the in-memory one stays at 0 — and the clone's preview enqueue, guarded on `board_images.any?`, was dead from the day it was added: **no board cloned onto a communicator ever had a cover rendered.** `Boards::SeededSetCloner` hit the same trap and works around it with `root.reload`; `clone_with_images` now does the same, which also clears the loaded-empty association cache `count_records` poisons on the object handed back to `Boards::AssignmentCloner`. Two corollaries: a clone must not inherit the source's `settings["preset_display_image_url"]` (the column is nulled on clone, so `Board#preset_display_image_url` would fall through to the SOURCE board's cover) nor its `preview_status`/`preview_generated_at` (claiming a render it never had, on the field clients poll); and `rake board_covers:render_missing` clears the historical backlog, selecting by outcome — tiles, no preview attachment, blank column — rather than by provenance.
  - Testing note: `run_generate_preview_job` defers the Sidekiq push to `ActiveRecord.after_all_transactions_commit`, and under transactional fixtures that commit never arrives — so `GenerateBoardPreviewJob.jobs` is empty whether or not the enqueue fired. Assert on `settings["preview_status"] == "queued"`, which `mark_preview_queued!` writes synchronously.
